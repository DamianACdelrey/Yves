...
def build_demo(
    ...
    By default, it uses `:memory:`, so the database lives only in RAM and is
    deleted when the connection is closed. If you want a persistent database on
    disk, pass a file path in `db_path` (for example, `"demo.sqlite"`).
    It returns:
